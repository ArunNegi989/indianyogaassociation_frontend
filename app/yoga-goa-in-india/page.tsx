...
const CertIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    <rect x="2" y="3" width="20" height="14" rx="2" /><path d="M8 17v4M16 17v4M8 21h8M9 10l2 2 4-4" />
  </svg>
);
const StyleIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    <circle cx="12" cy="4" r="1.5" /><path d="M12 6v5.5M8.5 13c0 2 1.5 4 3.5 4.5 2-0.5 3.5-2.5 3.5-4.5M10 18l-1.5 3.5M14 18l1.5 3.5M7 11l5 2.5 5-2.5" />
  </svg>
);
const LangIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
function CourseInfoCard({ seats, currency, rate }: { seats: Batch[]; currency: Currency; rate: number }) {
  ...
  const details = [
    ...
    { icon: <StyleIcon />, label: "YOGA STYLE", value: "Multistyle", sub: "Hatha, Ashtanga & Vinyasa" },
      ...
      <div className={styles.icCard}>
        ...
        <div className={styles.icRight}>
          ...
          <div className={styles.icPriceRow}>
            <span className={styles.icPriceOld}>{fmt(originalPrice).amount}</span>
            <span className={styles.icPriceNew}>{fmt(startingPrice).amount}</span>
...
function PremiumSeatBooking() {
  ...
  return (
    <section className={styles.datesSection} id="dates">
      ...
      <h2 className={styles.psbMainTitle}>Yoga Teacher Training — Goa, India</h2>
      <p className={styles.psbSecSub}>Choose your course, dates & preferred accommodation — prices include tuition and meals</p>
      ...
      <div className={styles.psbLayout}>
        ...
        <div className={styles.psbLeftPanel}>
          ...
          <div className={styles.psbLph}>
            <span className={styles.psbLphTitle}>Select Your Batch — {activeTabLabel}</span>
          ...
          {loading ? (
            ...
            <p className={styles.psbNoBatches} style={{ color: "#8a2c00" }}>{error}</p>
          ) : seats.length === 0 ? (
            <p className={styles.psbNoBatches}>No upcoming batches for {activeTabLabel}.</p>
          ) : (
            <div className={styles.psbBatchGrid}>
              {seats.map((batch) => {
                ...
                const seatsPercent = full ? 100 : Math.max(5, (rem / batch.totalSeats) * 100);
        ...
        <div className={styles.psbRightPanel}>
          ...
          <div className={styles.psbRpHead}>
            ...
            <div className={styles.psbRpCourse}>{activeTabLabel} Yoga Teacher Training — Goa</div>
            <div className={styles.psbRpDur}>
              ...
              <span className={styles.psbRpDurTxt}>Arambol Beach, Goa, India</span>
            </div>
          ...
          <div className={styles.psbRpBody}>
            ...
            <div className={styles.psbPriceRow}>
              <div className={styles.psbPriceCard}>
                <div className={styles.psbPcAmt}>{selected ? fmt(selected.privatePrice).amount : "—"}<span className={styles.psbPcCur}>{currency}</span></div>
                <div className={styles.psbPcLbl}>Private Room</div>
              </div>
              <div className={styles.psbPriceCard}>
                <div className={styles.psbPcAmt}>{selected ? fmt(selected.twinPrice).amount : "—"}<span className={styles.psbPcCur}>{currency}</span></div>
            ...
            <div className={styles.psbPriceWide}>
              <div className={styles.psbPwLeft}>
                <span className={styles.psbPcAmt} style={{ fontSize: "1rem" }}>{selected ? fmt(selected.dormPrice).amount : "—"}</span>
            ...
            {selected ? (
              <a href={`/yoga-registration?batchId=${selected._id}&type=${activeTab}`} className={styles.psbBookBtn}>
...
function DailyScheduleSection({ rows, scheduleImage, superLabel, sectionTitle, API_BASE }: {
  rows: ScheduleRow[];
  scheduleImage: string;
  superLabel: string;
  sectionTitle: string;
      ...
      <div className={styles.scheduleContainer}>
        {scheduleSrc && (
          <div className={styles.schedVideoSide}>
            <img src={scheduleSrc} alt="Daily Schedule" className={styles.schedImg} />
        ...
        <div className={styles.schedCardsSide}>
          ...
          <div className={styles.schedGrid}>
            {rows.map((s, i) => (
              <div key={s._id} className={styles.schedChip} style={{ "--i": i } as React.CSSProperties}>
                <div className={styles.schedIconBox}>
                  <span className={styles.schedNum}>{String(i + 1).padStart(2, "0")}</span>
                </div>
...
/* ─── CURRICULUM CARDS ─── */
function CurriculumSection({ superLabel, sectionTitle, learnings, focusSectionTitle, focusBodyText, mainFocus }: {
  superLabel: string; sectionTitle: string; learnings: string[];
  focusSectionTitle: string; focusBodyText: string; mainFocus: string[];
}) {
  return (
    <section className={styles.curriculumSection}>
      <div className={`${styles.container} ${styles.currContainer}`}>
        ...
        <div className={`${styles.reveal} ${styles.currGrid}`}>
          {learnings.map((l, i) => (
            <div key={i} className={styles.currCard} style={{ "--i": i } as React.CSSProperties}
              onMouseMove={(e) => {
                const rect = e.currentTarget.getBoundingClientRect();
                e.currentTarget.style.setProperty("--x", `${e.clientX - rect.left}px`);
                e.currentTarget.style.setProperty("--y", `${e.clientY - rect.top}px`);
              }}>
              <div className={styles.currCardNumber}>{String(i + 1).padStart(2, "0")}</div>
...
function MandalaRing({ size = 300, opacity = 0.08 }: { size?: number; opacity?: number }) {
  ...
  const spokes = 24; const petals = 12;
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ opacity }} aria-hidden>
      <g stroke="#f15505" strokeWidth="0.7" fill="none">
        ...
        {Array.from({ length: petals }).map((_, i) => {
          const a = (i / petals) * 2 * Math.PI; const r = rings[1];
...
function MandalaFull({ size = 600, opacity = 0.05 }: { size?: number; opacity?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ opacity }} aria-hidden>
      <g stroke="#f15505" strokeWidth="0.6" fill="none" transform={`translate(${c},${c})`}>
        {[0.46, 0.38, 0.3, 0.22, 0.14, 0.07].map((r, i) => <circle key={i} cx={0} cy={0} r={r * size} />)}
        {Array.from({ length: 36 }).map((_, i) => {
          const a = (i / 36) * 2 * Math.PI; const r0 = size * 0.07, r1 = size * 0.46;
        ...
        {[8, 16].map((n, ni) => Array.from({ length: n }).map((_, i) => {
          const a = (i / n) * 2 * Math.PI; const r = size * (ni === 0 ? 0.32 : 0.2);
...
export default function GoaYogaPage() {
  ...
  if (pageError || !pageData) {
    return (
      <div className={styles.page} style={{ minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div className={styles.stateBox}>Failed to load page. Please refresh.</div>
      </div>
    );
  }
  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageWatermark} aria-hidden="true"><MandalaFull size={700} opacity={0.03} /></div>

      {/* ════════ HERO ════════ */}
      <section className={styles.heroSection}>
        {heroSrc && <img src={heroSrc} alt={pageData.heroAlt} className={styles.heroImage} />}
      </section>
        ...
        <div className={`${styles.container} ${styles.galleryContainer}`}>
          ...
          <div className={`${styles.reveal} ${styles.campusGrid}`}>
            {pageData.campusImages?.map((img) => {
              ...
              return (
                <button key={img._id} className={styles.campusCard}
                  onClick={() => src && setModal({ src, label: img.label })}
                  aria-label={`View ${img.label}`}>
                  {src ? <img src={src} alt={img.label} /> : <div className={styles.campusPlaceholder}>🖼️</div>}
    ...
    <div className={`${styles.reveal} ${styles.infoGrid}`}>
      ...
      <div className={styles.infoCard}>
        ...
        <address className={styles.address}>
          ...
          <div className={styles.contactLinks}>
            {pageData.phone1 && <a href={`tel:${pageData.phone1}`}>{pageData.phone1}</a>}
            {pageData.phone2 && <a href={`tel:${pageData.phone2}`}>{pageData.phone2}</a>}
          </div>
        </address>
      </div>
      ...
      <div className={styles.infoCard}>
        ...
        <p className={styles.para}>
          {pageData.reachViaAirLabel && <strong>{pageData.reachViaAirLabel} </strong>}
      ...
      <div className={styles.infoCard}>
        ...
        <div className={styles.formFields}>
         ...
         {pageData.applyEmail && (
          <a href={`mailto:${pageData.applyEmail}`} className={styles.ctaBtn}>
            Apply Now →
          </a>
        )}
      </div>
  ...
  {pageData.rulesHref && (
    <Link href={pageData.rulesHref} className={styles.rulesBtn}>
      ...
      <section className={styles.footerCta}>
        <div className={styles.footerMandala} aria-hidden="true"><MandalaFull size={500} opacity={0.1} /></div>
        <div className={styles.container}>
          <div className={styles.footerCtaInner}>
            ...
            <div className={styles.heroBtns}>
              <a href={pageData.footerCtaDatesHref || "#dates"} className={styles.btnPrimary}>Dates</a>
              <a href={pageData.footerCtaEmailHref || `mailto:${pageData.applyEmail}`} className={styles.btnOutline}>Book Now</a>
            </div>
          </div>
        </div>
      </section>
      ...
      {modal && (
        <div className={styles.modalOverlay} onClick={() => setModal(null)} role="dialog" aria-modal="true">
          <div className={styles.modalBox} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalCaption}><OmDivider /><p>{modal.label}</p></div>
          </div>
        </div>
      )}
    </div>
