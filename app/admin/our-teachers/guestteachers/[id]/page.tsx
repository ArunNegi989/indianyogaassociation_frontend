...
export default function EditGuestTeacherPage() {
    ...
    const fetchTeacher = async () => {
      try {
        ...
        if (!t) {
          ...
          router.push("/admin/our-teachers/guestteachers");
          return;
        }
        setForm({
          name: t.name || "",
        ...
        toast.error("Failed to load guest teacher data");
        router.push("/admin/our-teachers/guestteachers");
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ✅ FIXED: send bio as indexed array
      form.bioItems
        .filter((b) => b.trim())
        .forEach((b, i) => fd.append(`bio[${i}]`, b.trim()));

      if (imageFile) {
        fd.append("image", imageFile);
      }

      // ✅ FIXED: correct guest teacher update endpoint
      await api.put(`/guest-teachers/update-guest-teacher/${teacherId}`, fd, {
    ...
    <div className={styles.page}>
      ...
      <div className={styles.breadcrumb}>
        <button
          ...
          onClick={() => router.push("/admin/our-teachers/guestteachers")}
        >
          Guest Teachers
        </button>
        <span className={styles.breadcrumbSep}>›</span>
        <span className={styles.breadcrumbCurrent}>
          Edit — {form.name || "Guest Teacher"}
        </span>
      </div>

      <div className={styles.pageHeader}>
        <div>
          <h1 className={styles.pageTitle}>Edit Guest Teacher</h1>
          <p className={styles.pageSubtitle}>
            Update profile for {form.name || "this guest teacher"}
          </p>
        </div>
      </div>
          ...
          <div className={styles.fieldGroup}>
            ...
            <div className={`${styles.inputWrap} ${errors.name ? styles.inputError : ""} ${form.name && !errors.name ? styles.inputSuccess : ""}`}>
              <input
                ...
                placeholder="e.g. Swami Ananda Ji"
                value={form.name}
                maxLength={100}
