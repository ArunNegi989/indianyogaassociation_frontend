...
export default function Goa500hrSeatsListPage() {
  ...
  const fetchRows = async () => {
    try {
      const res = await api.get("/goa-500hr-seats/getAllBatches");
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/goa-500hr-seats/deleteBatch/${deleteId}`);
