...
export default function Goa300hrSeatsListPage() {
  ...
  const fetchRows = async () => {
    try {
      const res = await api.get("/goa-300hr-seats/getAllBatches");
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/goa-300hr-seats/deleteBatch/${deleteId}`);
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.pageHeader}>
        <div>
          <h1 className={styles.pageTitle}>300 Hour Yoga TTC in GOA — Seats & Dates</h1>
          <p className={styles.pageSubtitle}>Manage 2025 & 2026 batch dates, fees and seat availability</p>
        </div>
        <Link href="/admin/yogacourse/yoga-goa-in-india/yoga-goa-300hr-seats/add-new" className={styles.addBtn}>
                ...
                <tr key={row._id} className={styles.row}>
                  ...
                  <td>
                    <div className={styles.actionBtns}>
                      <Link
                        href={`/admin/yogacourse/yoga-goa-in-india/yoga-goa-300hr-seats/${row._id}`}
