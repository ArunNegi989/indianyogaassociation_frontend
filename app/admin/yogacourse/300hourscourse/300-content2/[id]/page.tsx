...
/* ── Lazy Jodit (ref-based, uncontrolled — NO value prop, no freeze) ── */
function LazyJodit({
  label,
  hint,
  cr,
  ...
  return (
    <div className={styles.fieldGroup}>
      ...
      >
        {visible ? (
          /* ✅ NO value prop — uncontrolled to avoid freeze */
          <JoditEditor
            value={cr.current}
...
/* ── InlineJodit — uncontrolled, no value prop, no freeze ── */
const InlineJodit = ({
  onChange,
  ph = "Start typing…",
  h = 180,
  ...
  // Stable ref so Jodit never re-mounts on parent render
  ...
  return (
    <div
      ref={wrapRef}
      style={{ minHeight: h, border: "1px solid #e8d5b5", borderRadius: 8 }}
    >
      {visible ? (
        <JoditEditor
         value=""
      ...
      {items.map((item, i) => (
        ...
        >
          ...
          <div className={styles.nestedCardBody}>
            {/* ✅ No value prop — uncontrolled, no freeze */}
...
const removePara = (
  set: React.Dispatch<React.SetStateAction<ParaItem[]>>,
  id: string,
      ...
      >
        ...
        <label
          style={{
            ...
            height: 95,
            borderRadius: 8,
            border: "2px dashed rgba(224,123,0,0.4)",
            display: "flex",
        ...
        >
          <span style={{ fontSize: "1.4rem", color: "rgba(224,123,0,0.5)" }}>
            ＋
          </span>
          <span
            style={{
      ...
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          ...
          style={{
            ...
            fontSize: "1.6rem",
            lineHeight: 1,
            color: star <= (hovered || value) ? "#e8a800" : "#d9cfc0",
...
interface YouTubeItem {
  id: string;
  title: string;
  sourceType: "url" | "file"; // ✅ frontend field name is sourceType
  videoId: string;
  file: File | null;
  filePreview: string;
...
export default function Edit300hrContent2() {
  ...
  // ✅ NO useParams — single-record pattern, no ID needed
  ...
  const [evolutionParas, setEvolutionParas] = useState<ParaItem[]>([
    { id: "ep1", content: "" },
  ]);
  ...
  const [ethicsParas, setEthicsParas] = useState<ParaItem[]>([
    { id: "etp1", content: "" },
  ]);
  const [misconParas, setMisconParas] = useState<ParaItem[]>([
    { id: "mp1", content: "" },
  ]);
  ...
  const addImg =
    (s: React.Dispatch<React.SetStateAction<ImgItem[]>>) =>
    (f: File, p: string) =>
      s((prev) => [
        ...prev,
        { id: `img-${Date.now()}-${Math.random()}`, file: f, preview: p },
      ]);
  const removeImg =
    (s: React.Dispatch<React.SetStateAction<ImgItem[]>>) => (id: string) =>
      s((prev) => prev.filter((x) => x.id !== id));

  const updNested = useCallback(
    <T,>(
      arr: T[],
      set: (v: T[]) => void,
      id: string,
  ...
  const updYt = (id: string, key: keyof YouTubeItem, val: any) =>
    setYoutubeVideos((p) =>
      p.map((x) => (x.id === id ? { ...x, [key]: val } : x)),
    );
  const setYtFile = (id: string, f: File) =>
    setYoutubeVideos((p) =>
      p.map((x) =>
        x.id === id
          ? { ...x, file: f, filePreview: URL.createObjectURL(f) }
          : x,
      ),
    );
  const removeYtFile = (id: string) =>
    setYoutubeVideos((p) =>
      p.map((x) => (x.id === id ? { ...x, file: null, filePreview: "" } : x)),
    );
  ...
  /* ════════════════
     FETCH  ✅ GET /yoga-300hr/content2/  (no :id)
  ════════════════ */
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        // ✅ FIXED: was /yoga-300hr/content2/${id} — no id needed (single record)
        ...
        /* ── Jodit ref ── */
        if (d.markPracticalDetail)
          markPracticalDetailRef.current = d.markPracticalDetail;
        ...
        if (d.eligibilityParas)
          setEligibilityParas(toPara(d.eligibilityParas, "elp"));
        if (d.evaluationParas)
          setEvaluationParas(toPara(d.evaluationParas, "evp"));
  ...
  Array.isArray(arr) && arr.length ? arr : [];
        ...
        if (d.scheduleItems?.length)
          setScheduleItems(
            d.scheduleItems.map((s: any, i: number) => ({
              id: `s${i}`,
              time: s.time ?? "",
              activity: s.activity ?? "",
            })),
        ...
        /* ── YouTube videos ──
           Backend stores: { id, title, type, videoId, videoFile }
           Frontend state: { id, title, sourceType, videoId, file, filePreview, existingFileUrl }
           ✅ Map type → sourceType, videoFile → existingFileUrl
        ── */
        if (d.youtubeVideos?.length)
         setYoutubeVideos(d.youtubeVideos.map((yt: any, i: number) => ({
  id: yt.id || `yt${i}`,
  title: yt.title || "",   // ✅ ADD THIS
  ...
  /* ════════════════
     SUBMIT  ✅ PUT /yoga-300hr/content2/update  (no :id)
  ════════════════ */
  const onSubmit = async (data: FormData) => {
    try {
      ...
      fd.append(
        "evolutionParas",
        JSON.stringify(evolutionParas.map((p) => p.content)),
      );
      fd.append(
        "eligibilityParas",
        JSON.stringify(eligibilityParas.map((p) => p.content)),
      );
      fd.append(
        "evaluationParas",
        JSON.stringify(evaluationParas.map((p) => p.content)),
      );
      fd.append(
        "ethicsParas",
        JSON.stringify(ethicsParas.map((p) => p.content)),
      );
      fd.append(
        "misconParas",
        JSON.stringify(misconParas.map((p) => p.content)),
      );
      ...
      /* ── YouTube: send type (not sourceType) to match backend ── */
      const ytMeta = youtubeVideos.map((yt) => ({
        id: yt.id,
        type: yt.sourceType, // ✅ map sourceType → type for backend
        videoId: yt.videoId,
        existingFileUrl: yt.existingFileUrl ?? "",
      }));
      fd.append("youtubeVideosMeta", JSON.stringify(ytMeta));
      youtubeVideos.forEach((yt) => {
        if (yt.sourceType === "file" && yt.file)
          fd.append(`ytFile_${yt.id}`, yt.file);
      });

      /* ── Single images ── */
      if (diplomaFile) fd.append("diplomaImage", diplomaFile);
      else if (diplomaExisting && !diplomaPrev)
        fd.append("diplomaImageKeep", "true");

      if (yogaGardenFile) fd.append("yogaGardenImage", yogaGardenFile);
      else if (yogaGardenExisting && !yogaGardenPrev)
        fd.append("yogaGardenImageKeep", "true");

      /* ── Carousel images: new files + keep existing ── */
      ...
      // ✅ FIXED: was api.put(`/yoga-300hr/content2/${id}`, ...) — now correct endpoint
      await api.put("/yoga-300hr/content2/update", fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      setSubmitted(true);
      setTimeout(
        () => router.push("/admin/yogacourse/300hourscourse/300-content2"),
        1500,
      );
    } catch (e: any) {
  ...
  return (
    <div className={styles.formPage}>
      ...
      <div className={styles.pageHeader}>
        <div className={styles.pageHeaderText}>
          ...
          <p className={styles.pageSubtitle}>
            Evolution · FAQ · Accommodation · Schedule · Reviews
          </p>
        </div>
      </div>
      ...
      <div className={styles.formCard}>
        ...
        <Sec title="Evolution and Certification">
          <F label="Section H2 Heading">
            <div className={styles.inputWrap}>
              <input
                className={`${styles.input} ${styles.inputNoCount}`}
                {...register("evolutionH2")}
              />
            </div>
          </F>
          <F
            label="Introduction Paragraphs"
            hint="Add as many paragraphs as needed."
            req
          >
            <DynamicParaList
          ...
          >
            ...
            <F
              label="Mark Distribution Sub-text / Intro Line"
              hint="Short description shown below the H3 (optional)"
            >
              <div className={styles.inputWrap}>
                <input
                  className={`${styles.input} ${styles.inputNoCount}`}
                  {...register("markDistSubText")}
                  placeholder="e.g. The examination is divided as follows…"
                />
              </div>
            </F>
            <div className={styles.grid2} style={{ marginTop: "0.8rem" }}>
              <F label="Total Marks — Label">
                <div className={styles.inputWrap}>
                  <input
                    className={`${styles.input} ${styles.inputNoCount}`}
                    {...register("markTotalLabel")}
                  />
                </div>
              </F>
              <F label="Total Marks — Text / Value">
                <div className={styles.inputWrap}>
                  <input
                    className={`${styles.input} ${styles.inputNoCount}`}
                    {...register("markTotalText")}
                    placeholder="e.g. 200 Marks"
                  />
                </div>
              </F>
            </div>
            <div className={styles.grid2}>
              <F label="Theory Examination — Label">
                <div className={styles.inputWrap}>
                  <input
                    className={`${styles.input} ${styles.inputNoCount}`}
                    {...register("markTheoryLabel")}
                  />
                </div>
              </F>
              <F label="Theory Examination — Text / Value">
                <div className={styles.inputWrap}>
                  <input
                    className={`${styles.input} ${styles.inputNoCount}`}
                    {...register("markTheoryText")}
                    placeholder="e.g. 60 Marks"
                  />
                </div>
              </F>
            </div>
            <div className={styles.grid2}>
              <F label="Practical Examination — Label">
                <div className={styles.inputWrap}>
                  <input
                    className={`${styles.input} ${styles.inputNoCount}`}
                    {...register("markPracticalLabel")}
                  />
                </div>
              </F>
              <F label="Practical Examination — Text / Value">
                <div className={styles.inputWrap}>
                  <input
                    className={`${styles.input} ${styles.inputNoCount}`}
                    {...register("markPracticalText")}
                    placeholder="e.g. 140 Marks"
                  />
                </div>
              </F>
            </div>
        ...
        <Sec title="Career Opportunities & Fee Cards">
          ...
          <F label="Career Items">
            <StrList
              items={careerItems}
              label="Career"
              ph="Yoga Instructor at the health clubs"
              onAdd={() => setCareerItems((p) => [...p, ""])}
              onRemove={(i) =>
                setCareerItems((p) => p.filter((_, x) => x !== i))
              }
              onUpdate={(i, v) => {
                const a = [...careerItems];
                a[i] = v;
                ...
                <div className={styles.nestedCardBody}>
                  ...
                  <F label="Card Items">
                    <StrList
                      items={feeCard1Items}
                      label="Item"
                      ph="Online Course Fee: 25,000 INR"
                      onAdd={() => setFeeCard1Items((p) => [...p, ""])}
                      onRemove={(i) =>
                        setFeeCard1Items((p) => p.filter((_, x) => x !== i))
                      }
                      onUpdate={(i, v) => {
                        const a = [...feeCard1Items];
                        a[i] = v;
                        setFeeCard1Items(a);
                      }}
                    />
                ...
                <div className={styles.nestedCardBody}>
                  ...
                  <F label="Card Items">
                    <StrList
                      items={feeCard2Items}
                      label="Item"
                      ph="Offline Fee: 25,000 INR - Dorm Shared"
                      onAdd={() => setFeeCard2Items((p) => [...p, ""])}
                      onRemove={(i) =>
                        setFeeCard2Items((p) => p.filter((_, x) => x !== i))
                      }
                      onUpdate={(i, v) => {
                        const a = [...feeCard2Items];
                        a[i] = v;
                        setFeeCard2Items(a);
                      }}
                    />
                ...
                <div className={styles.nestedCardHeader}>
                  ...
                  {faqItems.length > 1 && (
                    <button
                      ...
                      className={styles.removeNestedBtn}
                      onClick={() =>
                        setFaqItems((p) => p.filter((x) => x.id !== faq.id))
                      }
                    >
                      ✕ Remove
                    </button>
                  ...
                  <F label="Question">
                    <div className={styles.inputWrap}>
                      <input
                        className={`${styles.input} ${styles.inputNoCount}`}
                        value={faq.question}
                        placeholder="Why 300 hour yoga TTC in Rishikesh at AYM?"
                        onChange={(e) =>
                          updNested(
                            faqItems,
                            setFaqItems,
        ...
        <Sec
          title="Accommodation Carousel Images"
          badge={`${accomImages.length} images`}
        >
          <F label="Section H3">
            <div className={styles.inputWrap}>
              <input
          ...
          <F
            ...
            hint="Existing images shown. Add new or remove existing."
          >
            <MultiImg
              items={accomImages}
              onAdd={addImg(setAccomImages)}
              onRemove={removeImg(setAccomImages)}
              hint="Add image"
              label="accommodation images"
            />
          </F>
        ...
        <Sec title="Food Carousel Images" badge={`${foodImages.length} images`}>
          ...
          >
            <MultiImg
              items={foodImages}
              onAdd={addImg(setFoodImages)}
              onRemove={removeImg(setFoodImages)}
              hint="Add image"
              label="food images"
            />
          </F>
        ...
        <Sec title="Luxury Room & Features">
          ...
          <F label="Luxury Features List">
            <StrList
              items={luxuryFeatures}
              label="Feature"
              ph="Swimming pool"
              onAdd={() => setLuxuryFeatures((p) => [...p, ""])}
              onRemove={(i) =>
                setLuxuryFeatures((p) => p.filter((_, x) => x !== i))
              }
              onUpdate={(i, v) => {
                const a = [...luxuryFeatures];
                a[i] = v;
                setLuxuryFeatures(a);
              }}
            />
          ...
          <F label="Luxury Room Images" hint="Recommended 500×400px">
            <MultiImg
              items={luxuryImages}
              onAdd={addImg(setLuxuryImages)}
              onRemove={removeImg(setLuxuryImages)}
              hint="Add image"
              label="luxury images"
            />
          </F>
          <F
            label="Yoga Garden / Full-Width Banner Image"
            hint="Wide banner image below luxury features"
          >
            <SingleImg
              ...
              onRemove={() => {
                setYogaGardenFile(null);
                setYogaGardenPrev("");
                setYogaGardenExisting("");
              }}
            />
          </F>
        </Sec>
        ...
        <Sec title="Features & Daily Schedule">
          ...
          <F label="Features List">
            <StrList
              items={featuresList}
              label="Feature"
              ph="Students with basic, intermediate, or advanced knowledge…"
              onAdd={() => setFeaturesList((p) => [...p, ""])}
              onRemove={(i) =>
                setFeaturesList((p) => p.filter((_, x) => x !== i))
              }
              onUpdate={(i, v) => {
                const a = [...featuresList];
                a[i] = v;
                setFeaturesList(a);
              }}
            />
              ...
              >
                ...
                <div className={`${styles.inputWrap} ${styles.listInput}`}>
                  <input
                    className={`${styles.input} ${styles.inputNoCount}`}
                    value={s.activity}
                    placeholder="Pranayama And Meditation"
                    onChange={(e) =>
                      updNested(
                        scheduleItems,
                        setScheduleItems,
                ...
                <button
                  ...
                  className={styles.removeItemBtn}
                  onClick={() =>
                    setScheduleItems((p) => p.filter((x) => x.id !== s.id))
                  }
          ...
          <F
            label="Schedule Side Images"
            hint="3 images shown beside the schedule table"
          >
            <MultiImg
              items={scheduleImages}
              onAdd={addImg(setScheduleImages)}
              onRemove={removeImg(setScheduleImages)}
              hint="Add image"
        ...
        <Sec title="Learning Outcomes">
          ...
          <F
            ...
            hint="Each item is displayed as a separate point below the heading."
          >
            <StrList
              items={learningItems}
              label="Outcome"
              ph="Upon completion, students will be awarded the prestigious 300-hour YTTC certification…"
              onAdd={() => setLearningItems((p) => [...p, ""])}
              onRemove={(i) =>
                setLearningItems((p) => p.filter((_, x) => x !== i))
              }
              onUpdate={(i, v) => {
                const a = [...learningItems];
                a[i] = v;
                setLearningItems(a);
              }}
            />
        ...
        <Sec title="Eligibility Section">
          <F label="Section H2 Heading">
            <div className={styles.inputWrap}>
              <input
                className={`${styles.input} ${styles.inputNoCount}`}
                {...register("eligibilityH2")}
              />
            </div>
          </F>
          <F
            label="Eligibility Tag"
            hint="Short tag label shown above the paragraph, e.g. 'Eligibility'"
          >
            <div className={styles.inputWrap}>
              <input
                className={`${styles.input} ${styles.inputNoCount}`}
                {...register("eligibilityTag")}
        ...
        <Sec title="Evaluation and Certification">
          <F label="Section H2">
            <div className={styles.inputWrap}>
              <input
                className={`${styles.input} ${styles.inputNoCount}`}
                {...register("evaluationH2")}
              />
            </div>
          </F>
          <F
            label="Evaluation Paragraphs"
            hint="Add as many paragraphs as needed."
            req
          >
            <DynamicParaList
        ...
        <Sec title="Yoga Ethics — Code of Conduct">
          ...
          <F
            label="Additional Ethics Introduction Paragraphs (optional)"
          ...
          <F label="Ethics Quote" hint="Displayed in styled quote block">
            <div className={styles.inputWrap}>
              <textarea
                className={`${styles.input} ${styles.textarea} ${styles.inputNoCount}`}
                rows={2}
                {...register("ethicsQuote")}
              />
            </div>
          </F>
          <F
            label="Naturalistic Power of Yoga — Intro Paragraph"
            hint="The paragraph starting 'According to yoga gurus…'"
          >
            <div className={styles.inputWrap}>
              <textarea
                className={`${styles.input} ${styles.textarea} ${styles.inputNoCount}`}
          ...
          <F label="Ethics Rules / Guidelines">
            <StrList
              items={ethicsRules}
              label="Rule"
              ph="Students need to be obedient in classes…"
              onAdd={() => setEthicsRules((p) => [...p, ""])}
              onRemove={(i) =>
                setEthicsRules((p) => p.filter((_, x) => x !== i))
              }
              onUpdate={(i, v) => {
                const a = [...ethicsRules];
                a[i] = v;
          ...
          <F
            label="Diploma / Graduation Image"
            hint="Full-width image below ethics rules"
          >
            <SingleImg
              preview={diplomaPrev}
        ...
        <Sec title="Misconceptions Section">
          ...
          <F
            label="Shake Your Myths — Intro Paragraphs (optional)"
          ...
          <F label="Misconception Items">
            <StrList
              items={misconItems}
              label="Misconception"
              ph="You think you are signing up to learn yoga…"
              onAdd={() => setMisconItems((p) => [...p, ""])}
              onRemove={(i) =>
                setMisconItems((p) => p.filter((_, x) => x !== i))
              }
              onUpdate={(i, v) => {
                const a = [...misconItems];
                a[i] = v;
        ...
        <Sec
          title="Student Reviews & YouTube Videos"
          badge={`${reviews.length} reviews · ${youtubeVideos.length} videos`}
        >
          <div className={styles.grid2}>
            <F label="Section H2">
              <div className={styles.inputWrap}>
                ...
                <div className={styles.nestedCardBody}>
                  ...
                  <F label="Star Rating">
                    <StarRating
                      value={r.rating}
                      onChange={(v) =>
                        updNested(reviews, setReviews, r.id, "rating", v)
                      }
                    />
                  </F>
                  <F label="Review Text">
                    <div className={styles.inputWrap}>
                      <textarea
                        ...
                        rows={4}
                        value={r.text}
                        placeholder="My experience at AYM Yoga School was amazing!…"
                        onChange={(e) =>
                          updNested(
                            reviews,
                            setReviews,
          ...
          <F
            ...
            hint="Each video supports either a YouTube URL link or a direct video file upload."
          >
                ...
                <div className={styles.nestedCardHeader}>
                  ...
                  {youtubeVideos.length > 1 && (
                    <button
                      ...
                      className={styles.removeNestedBtn}
                      onClick={() =>
                        setYoutubeVideos((p) => p.filter((x) => x.id !== yt.id))
                      }
                    >
                      ✕ Remove
                    </button>
                ...
                <div className={styles.nestedCardBody}>
                  <F label="Video Title">
                    <div className={styles.inputWrap}>
                      <input
                        className={`${styles.input} ${styles.inputNoCount}`}
                        value={yt.title}
                        placeholder="300 Hour Yoga TTC Review by Alex…"
                    ...
                    >
                      {(["url", "file"] as const).map((opt) => (
                        ...
                        >
                          <input
                            type="radio"
                            name={`ytSrcType-${yt.id}`}
                            checked={yt.sourceType === opt}
                            onChange={() => updYt(yt.id, "sourceType", opt)}
                  ...
                  {yt.sourceType === "url" && (
                    <F
                      ...
                      hint="e.g. pXU4_SXdNdY — the part after watch?v="
                    >
                      <div className={styles.inputWrap}>
                        <input
                          className={`${styles.input} ${styles.inputNoCount}`}
                          value={yt.videoId}
                          placeholder="pXU4_SXdNdY"
                          onChange={(e) =>
                            updYt(yt.id, "videoId", e.target.value)
                          }
                        />
                      </div>
                      {yt.videoId && (
                  ...
                  {yt.sourceType === "file" && (
                    <F
                      ...
                      hint="MP4/WebM/MOV · Max recommended 200MB"
                    >
          ...
          <div className={styles.grid2}>
            <F label="Slug" req>
              ...
              >
                <input
                  className={`${styles.input} ${styles.inputNoCount}`}
                  placeholder="300-hour-yoga-ttc-content2"
      ...
      <div className={styles.formActions}>
        <Link
          href="/admin/yogacourse/300hourscourse/300-content2"
