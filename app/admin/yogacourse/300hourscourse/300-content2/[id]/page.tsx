    ...
    <div className={styles.formPage}>
      ...
      <div className={styles.formCard}>
        ...
        <Sec title="Accommodation Carousel Images" badge={`${accomImages.length} images`}>
          ...
          <F label="Accommodation Images" hint="Existing images shown. Add new or remove existing.">
        ...
        <Sec title="Food Carousel Images" badge={`${foodImages.length} images`}>
          ...
          <F label="Food Images" hint="Existing images shown. Add new or remove existing.">
        ...
        <Sec title="Yoga Ethics — Code of Conduct">
          ...
          <F label="Diploma / Graduation Image" hint="Full-width image below ethics rules">
            <SingleImg
              preview={diplomaPrev} existingUrl={diplomaExisting} badge="Diploma" hint="Students with diploma image"
                    ...
                    <div style={{ display: "flex", gap: "1.2rem", alignItems: "center", flexWrap: "wrap" }}>
                      {(["url", "file"] as const).map((opt) => (
                        <label key={opt} style={{ display: "flex", alignItems: "center", gap: "0.4rem", cursor: "pointer", fontFamily: "Cormorant Garamond,serif", fontSize: "0.92rem", color: "#5a3a1a" }}>
                          <input type="radio" name={`ytSrcType-${yt.id}`} checked={yt.sourceType === opt} onChange={() => updYt(yt.id, "sourceType", opt)} style={{ width: 14, height: 14 }} />
                    ...
                    <F label="Upload Video File" hint="MP4/WebM/MOV · Max recommended 200MB">
                      {!yt.filePreview && yt.existingFileUrl && (
                        <div style={{ marginBottom: "0.6rem" }}>
                          <p style={{ fontFamily: "Cormorant Garamond,serif", fontSize: "0.82rem", color: "#7a5c3a", fontStyle: "italic", marginBottom: "0.3rem" }}>Current video:</p>
                          <video src={`${BASE_URL}${yt.existingFileUrl}`} controls style={{ maxWidth: 280, borderRadius: 8, border: "1px solid #e8d5b5" }} />
                        </div>
                      )}
                      {!yt.filePreview ? (
