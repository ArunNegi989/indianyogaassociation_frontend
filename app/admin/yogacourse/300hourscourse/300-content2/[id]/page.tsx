...
/* ── Inline Jodit (value-based, for dynamic para arrays) ── */
function InlineJodit({ value, onChange, ph = "Start typing…", h = 180 }: {
  value: string; onChange: (v: string) => void; ph?: string; h?: number;
}) {
  const [visible, setVisible] = useState(false);
  const wrapRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
  ...
  return (
    <div ref={wrapRef} style={{ minHeight: h, border: "1px solid #e8d5b5", borderRadius: 8 }}>
      {visible ? (
        <JoditEditor value={value} config={{ ...joditConfig, placeholder: ph, height: h }} onChange={onChange} />
      ) : (
      ...
      {items.map((item, i) => (
        <div key={item.id} className={styles.nestedCard} style={{ marginBottom: "0.8rem" }}>
          <div className={styles.nestedCardHeader}>
            ...
            {items.length > 1 && (
              <button type="button" className={styles.removeNestedBtn} onClick={() => onRemove(item.id)}>✕ Remove</button>
            )}
          </div>
          <div className={styles.nestedCardBody}>
            <InlineJodit value={item.content} onChange={v => onUpdate(item.id, v)} ph={ph} />
          </div>
        </div>
      ))}
...
export default function Edit300hrContent2() {
  ...
  const [youtubeVideos, setYoutubeVideos] = useState<YouTubeItem[]>([
    { id: "yt1", title: "", type: "url", videoId: "", file: null, filePreview: "" },
  ]);
  ...
  return (
    <div className={styles.formPage}>
      <div className={styles.breadcrumb}>
        <button className={styles.breadcrumbLink} onClick={() => router.push("/admin/yogacourse/300hourscourse/300hr-content2")}>
      ...
      <div className={styles.formCard}>
          ...
          <div style={{ marginTop: "1.5rem", padding: "1.2rem", background: "#faf8f4", borderRadius: 10, border: "1px solid #e8d5b5" }}>
            ...
            <div className={styles.grid2} style={{ marginTop: "0.8rem" }}>
              ...
              <F label="Total Marks — Text / Value">
                <div className={styles.inputWrap}><input className={`${styles.input} ${styles.inputNoCount}`} {...register("markTotalText")} placeholder="e.g. 200 Marks" /></div>
              </F>
            </div>
            ...
            <div className={styles.grid2}>
              ...
              <F label="Theory Examination — Text / Value">
                <div className={styles.inputWrap}><input className={`${styles.input} ${styles.inputNoCount}`} {...register("markTheoryText")} placeholder="e.g. 60 Marks" /></div>
              </F>
            </div>
            ...
            <div className={styles.grid2}>
              ...
              <F label="Practical Examination — Text / Value">
                <div className={styles.inputWrap}><input className={`${styles.input} ${styles.inputNoCount}`} {...register("markPracticalText")} placeholder="e.g. 140 Marks" /></div>
              </F>
            </div>
          ...
          <F label="FAQ Items">
            {faqItems.map((faq, i) => (
              <div key={faq.id} className={styles.nestedCard} style={{ marginBottom: "0.8rem" }}>
                ...
                <div className={styles.nestedCardBody}>
                  <F label="Question">
                    <div className={styles.inputWrap}><input className={`${styles.input} ${styles.inputNoCount}`} value={faq.question}
                  ...
                  <F label="Answer">
                    <div className={styles.inputWrap}><textarea className={`${styles.input} ${styles.textarea} ${styles.inputNoCount}`} rows={3}
                      value={faq.answer} placeholder="AYM Yoga School offers…"
            ...
            {scheduleItems.map((s, i) => (
              <div key={s.id} className={styles.listItemRow} style={{ marginBottom: "0.5rem", gap: "0.5rem" }}>
                ...
                <button type="button" className={styles.removeItemBtn}
                  onClick={() => setScheduleItems(p => p.filter(x => x.id !== s.id))}
                  disabled={scheduleItems.length <= 1}>✕</button>
              </div>
            ))}
            <button type="button" className={styles.addItemBtn}
        ...
        <Sec title="Yoga Ethics — Code of Conduct">
          ...
          <F label="Naturalistic Power of Yoga — Intro Paragraph"
            ...
            <div className={styles.inputWrap}>
              <textarea className={`${styles.input} ${styles.textarea} ${styles.inputNoCount}`} rows={3}
        ...
        <Sec title="Student Reviews & YouTube Videos"
            ...
            {reviews.map((r, i) => (
              <div key={r.id} className={styles.nestedCard} style={{ marginBottom: "0.8rem" }}>
                <div className={styles.nestedCardHeader}>
                  ...
                  {reviews.length > 1 && (
                    <button type="button" className={styles.removeNestedBtn}
                      onClick={() => setReviews(p => p.filter(x => x.id !== r.id))}>✕ Remove</button>
                  )}
                </div>
                <div className={styles.nestedCardBody}>
                  <div className={styles.grid2}>
                    <F label="Name">
                      <div className={styles.inputWrap}><input className={`${styles.input} ${styles.inputNoCount}`} value={r.name}
                        placeholder="Karina Miller"
                        onChange={e => updNested(reviews, setReviews, r.id, "name", e.target.value)} /></div>
                    </F>
                    <F label="Role">
                      <div className={styles.inputWrap}><input className={`${styles.input} ${styles.inputNoCount}`} value={r.role}
                  ...
                  <F label="Review Text">
                    <div className={styles.inputWrap}>
                      <textarea className={`${styles.input} ${styles.textarea} ${styles.inputNoCount}`} rows={4}
                        value={r.text} placeholder="My experience at AYM Yoga School was amazing!…"
          ...
          <F label="YouTube Videos" hint="Each video supports either a YouTube URL link or a direct video file upload.">
              ...
              <div key={yt.id} className={styles.nestedCard} style={{ marginBottom: "0.8rem" }}>
                <div className={styles.nestedCardHeader}>
                  ...
                  {youtubeVideos.length > 1 && (
                    <button type="button" className={styles.removeNestedBtn}
                      onClick={() => setYoutubeVideos(p => p.filter(x => x.id !== yt.id))}>✕ Remove</button>
                  )}
                </div>
                <div className={styles.nestedCardBody}>
                  <F label="Video Title">
                  ...
                  {yt.type === "url" && (
                    <F label="YouTube Video ID" hint="e.g. pXU4_SXdNdY — the part after watch?v=">
                      <div className={styles.inputWrap}>
                        <input className={`${styles.input} ${styles.inputNoCount}`} value={yt.videoId}
            ...
            <button type="button" className={styles.addItemBtn}
              onClick={() => setYoutubeVideos(p => [...p, { id: `yt-${Date.now()}`, title: "", type: "url", videoId: "", file: null, filePreview: "" }])}>
              ＋ Add Video
            </button>
          </F>
        </Sec><D />
