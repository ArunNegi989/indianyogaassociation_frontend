// FILE: src/app/admin/yogacourse/vinyasa-yoga-course/vinyasa-seats/page.tsx
"use client";
...
export default function VinyasaSeatsListPage() {
  ...
  const fetchRows = async () => {
    try {
      const res = await api.get("/vinyasa-seats");
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/vinyasa-seats/${deleteId}`);
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.pageHeader}>
        <div>
          <h1 className={styles.pageTitle}>Vinyasa Yoga — Seats & Dates</h1>
          <p className={styles.pageSubtitle}>
            Manage upcoming vinyasa batch dates, fees and seat availability
          </p>
        </div>
        <Link
          href="/admin/yogacourse/vinyasa-yoga-course/vinyasa-seats/add-new"
                ...
                <tr key={row._id} className={styles.row}>
                  ...
                  <td>
                    <div className={styles.actionBtns}>
                      <Link
                        href={`/admin/yogacourse/vinyasa-yoga-course/vinyasa-seats/${row._id}`}
