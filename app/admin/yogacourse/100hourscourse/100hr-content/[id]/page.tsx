...
const toV = (arr: unknown): { value: string }[] => {
  if (!Array.isArray(arr) || arr.length === 0) return [{ value: "" }];
  return (arr as string[]).map((v) => ({ value: typeof v === "string" ? v : "" }));
};
...
const DEFAULT_WHY_CARDS: WhyChooseCard[] = [
...
const toWhyCards = (val: unknown): WhyChooseCard[] =>
  Array.isArray(val) && val.length > 0 ? (val as WhyChooseCard[]) : DEFAULT_WHY_CARDS;
...
const EMPTY: FormData = {
  bannerImage: "",
  heroTitle: "", heroParagraphs: [{ value: "" }],
  videoUrl: "", videoFile: "",
  transformTitle: "", transformParagraphs: [{ value: "" }], transformImage: "",
  whatIsTitle: "", whatIsParagraphs: [{ value: "" }],
  whyChooseTitle: "", whyChooseParagraphs: [{ value: "" }], whyChooseCards: DEFAULT_WHY_CARDS,
  ...
  syllabusImage1: "", syllabusImage2: "",
  syllabusVideoUrl: "", syllabusVideoFile: "",
  scheduleImage: "",
  scheduleVideoUrl: "", scheduleVideoFile: "",
  scheduleItems: [{ time: "", label: "" }],
  soulShineText: "", soulShineImage: "",
  enrollTitle: "", enrollParagraphs: [{ value: "" }], enrollItems: [{ value: "" }], enrollImage: "",
  comprehensiveTitle: "", comprehensiveParagraphs: [{ value: "" }],
  comprehensiveVideoUrl: "", comprehensiveVideoFile: "",
  certTitle: "", certParagraphs: [{ value: "" }], certImage: "",
  registrationTitle: "", registrationParagraphs: [{ value: "" }], registrationImage: "",
  includedItems: [{ value: "" }], notIncludedItems: [{ value: "" }],
  courseDuration: "13 Days", courseLevel: "Beginner", courseCertification: "100 Hour",
  courseYogaStyle: "Multistyle", courseYogaStyleSub: "Ashtanga, Vinyasa & Hatha",
  courseLanguage: "English & Hindi", courseDateInfo: "1st to 13th of every month",
  courseStartingFeeUSD: 499, courseOriginalFeeUSD: 899, bookNowLink: "#dates-fees",
};

/* ══════════════════════════════════════════════════════
   SUB-COMPONENTS  (identical to Add form)
══════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════
   VIDEO FIELD WITH TABS — each section has its OWN urlField & fileField
══════════════════════════════════════════════════════ */
function VideoFieldWithTabs({
  label, hint, videoUrl, videoFile,
  fieldNameUrl, fieldNameFile,
  fileRef, inputRef, setValue, onFileChange,
}: {
  label: string; hint?: string;
  videoUrl: string;
  videoFile: string;
  fieldNameUrl:  keyof Pick<FormData, "videoUrl"|"syllabusVideoUrl"|"scheduleVideoUrl"|"comprehensiveVideoUrl">;
  fieldNameFile: keyof Pick<FormData, "videoFile"|"syllabusVideoFile"|"scheduleVideoFile"|"comprehensiveVideoFile">;
  fileRef:  React.MutableRefObject<File | null>;
  inputRef: React.MutableRefObject<HTMLInputElement | null>;
  setValue: UseFormSetValue<FormData>;
  onFileChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
}) {
  // Determine initial tab: if a file path exists (not a URL), show upload tab
    ...
    <div className={styles.fieldGroup}>
      ...
      {tab === "link" && (
        <>
          ...
          {videoUrl && (
            <button type="button" className={styles.imgClearBtn} style={{ marginTop: 4 }}
              onClick={() => setValue(fieldNameUrl, "")}>✕</button>
          )}
          {embedSrc && (
      ...
      {tab === "upload" && (
        <>
          <div className={styles.videoUploadZone} onClick={() => inputRef.current?.click()}
            style={{ cursor: "pointer" }}>
            <span>📁 Click to upload MP4 video</span><br />
            <span className={styles.imgUploadSub}>MP4 recommended · max 100MB</span>
          </div>
          <input ref={inputRef} type="file" accept="video/mp4,video/*"
          ...
          {videoFile && (
            <div style={{ marginTop: 8, fontSize: "0.84rem", color: "#3d6000" }}>
              ...
              <button type="button" className={styles.imgClearBtn} style={{ marginLeft: 8 }}
                onClick={() => { fileRef.current = null; setValue(fieldNameFile, ""); }}>✕ Clear</button>
            </div>
          )}
          {/* Preview existing uploaded video */}
          {videoFile && (videoFile.startsWith("/uploads/") || videoFile.startsWith("http")) && (
            <video src={imgUrl(videoFile)} controls
...
function WhyChooseCardsField({
  ...
  const { fields, append, remove } = useFieldArray({ control, name: "whyChooseCards" });
  ...
  return (
    <div className={styles.fieldGroup}>
      ...
      <p className={styles.fieldHint}>Feature cards shown in the Why Choose section</p>
        ...
        {fields.map((field, index) => (
          <div key={field.id} className={styles.whyCardItem}>
            <div className={styles.whyCardHeader}>
              ...
              <button type="button" className={styles.removeItemBtn}
                onClick={() => remove(index)} disabled={fields.length <= 1}>✕</button>
            </div>
            <div className={styles.whyCardFields}>
              ...
              <div className={styles.fieldGroupSm}>
                ...
                <div className={`${styles.inputWrap} ${errors.whyChooseCards?.[index]?.label ? styles.inputError : ""}`}>
                  <input type="text" className={styles.input} placeholder="e.g., Expert Teachers"
...
export default function ContentEditPage() {
  ...
  /* ══════════════════════════════════════════════
     FETCH & PREFILL  — all data from backend
  ══════════════════════════════════════════════ */
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        /*
          Backend stores video as one of:
            d.videoUrl / d.videoFile   — main video
            d.syllabusVideo            — single field (URL or file path)
            d.scheduleVideo            — single field
            d.comprehensiveVideo       — single field

          We split each into URL vs File:
        */
        const parseVideo = (val: string | undefined) => {
          const v = val ?? "";
          if (!v) return { url: "", file: "" };
          if (v.startsWith("http") || v.includes("youtube") || v.includes("instagram")) {
            return { url: v, file: "" };
          }
          return { url: "", file: v }; // uploaded file path
        };

        const syllabusVid  = parseVideo(d.syllabusVideo);
        const scheduleVid  = parseVideo(d.scheduleVideo);
        const compVid      = parseVideo(d.comprehensiveVideo);
        ...
        reset({
          ...
          whyChooseCards:          toWhyCards(d.whyChooseCards),
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    try {
      ...
      imageMappings.forEach(({ file, field, urlField, dataKey }) => {
        if (file.current) {
          ...
          if (val && !val.startsWith("blob:")) {
            // Strip API prefix if present so backend stores clean relative path
            const clean = val.startsWith(API_URL) ? val.slice(API_URL.length) : val;
            fd.append(urlField, clean);
          }
        }
      });
      ...
      // Syllabus video  → backend field: syllabusVideo
      if (syllabusVideoFile.current) {
        ...
        fd.append("syllabusVideoFile", data.syllabusVideoFile || "");
      }

      // Schedule video  → backend field: scheduleVideo
      if (scheduleVideoFile.current) {
        ...
        fd.append("scheduleVideoFile", data.scheduleVideoFile || "");
      }

      // Comprehensive video  → backend field: comprehensiveVideo
      if (comprehensiveVideoFile.current) {
        ...
        fd.append("comprehensiveVideoFile", data.comprehensiveVideoFile || "");
      }
  ...
  return (
    <div className={styles.formPage}>
      <div className={styles.breadcrumb}>
        <Link href="/admin/yogacourse/100hourscourse/100hr-content" className={styles.breadcrumbLink}>
          Page Content
        </Link>
      ...
      <div className={styles.pageHeader}>
        ...
        <p className={styles.pageSubtitle}>Update all content sections for the 100 Hour YTT page</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        <div className={styles.formCard}>
          ...
          <div className={styles.sectionBlock}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
              <NField label="Starting Fee (USD)" hint="e.g. 499" name="courseStartingFeeUSD" register={fp.register} errors={fp.errors} />
              <NField label="Original / Crossed-out Fee (USD)" hint="e.g. 899" name="courseOriginalFeeUSD" register={fp.register} errors={fp.errors} />
            </div>
          ...
          <div className={styles.sectionBlock}>
            ...
            {/* Syllabus Video — own URL & File fields */}
            <VideoFieldWithTabs
          ...
          <div className={styles.sectionBlock}>
            ...
            <div className={styles.twoColGrid}>
              ...
              {/* Schedule Video — own URL & File fields */}
              <VideoFieldWithTabs
          ...
          <div className={styles.sectionBlock}>
            ...
            {/* Comprehensive Video — own URL & File fields */}
            <VideoFieldWithTabs
