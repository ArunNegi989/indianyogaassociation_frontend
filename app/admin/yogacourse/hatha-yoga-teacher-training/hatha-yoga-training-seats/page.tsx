// FILE: src/app/admin/dashboard/hathayoga-seats/page.tsx
"use client";
...
export default function HathaYogaSeatsListPage() {
  ...
  const fetchRows = async () => {
    try {
      const res = await api.get("/hathayoga-seats/getAllBatches");
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/hathayoga-seats/deleteBatch/${deleteId}`);
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.pageHeader}>
        <div>
          <h1 className={styles.pageTitle}>Hatha Yoga — Seats & Dates</h1>
          <p className={styles.pageSubtitle}>Manage upcoming batch dates, fees and seat availability</p>
        </div>
        <Link href="/admin/yogacourse/hatha-yoga-teacher-training/hatha-yoga-training-seats/add-new" className={styles.addBtn}>
                ...
                <tr key={row._id} className={styles.row}>
                  ...
                  <td>
                    <div className={styles.actionBtns}>
                      <Link
                        href={`/admin/yogacourse/hatha-yoga-teacher-training/hatha-yoga-training-seats/${row._id}`}
