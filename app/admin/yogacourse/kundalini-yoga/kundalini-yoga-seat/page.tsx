...
export default function KundaliniSeatsListPage() {
  ...
  const fetchRows = async () => {
    try {
      const res = await api.get("/kundalini-seats");
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/kundalini-seats/${deleteId}`);
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.pageHeader}>
        <div>
          <h1 className={styles.pageTitle}>Kundalini Yoga — Seats & Dates</h1>
          <p className={styles.pageSubtitle}>Manage upcoming batch dates, fees and seat availability</p>
        </div>
        <Link href="/admin/yogacourse/kundalini-yoga/kundalini-yoga-seat/add-new" className={styles.addBtn}>
                ...
                <tr key={row._id} className={styles.row}>
                  ...
                  <td>
                    <div className={styles.actionBtns}>
                      <Link
                        href={`/admin/yogacourse/kundalini-yoga/kundalini-yoga-seat/${row._id}`}
