// FILE: src/app/admin/dashboard/500hr-seats/page.tsx
"use client";
...
export default function Seats500ListPage() {
  ...
  try {
    const res = await api.get("/500hr-seats"); // ✅ fixed
  ...
  if (!deleteId) return;
  try {
    await api.delete(`/500hr-seats/${deleteId}`); // ✅ fixed
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.pageHeader}>
        <div>
          <h1 className={styles.pageTitle}>500 Hour — Seats & Dates</h1>
          <p className={styles.pageSubtitle}>
            Manage upcoming batch dates, fees and seat availability
          </p>
        </div>
        <Link
          href="/admin/yogacourse/500hourscourse/500hr-seats/add-new"
                ...
                <tr key={row._id} className={styles.row}>
                  ...
                  <td>
                    <div className={styles.roomChips}>
                      <span className={styles.roomChip}>
                        Dorm: <strong>${row.dormPrice}</strong>
                      </span>
                      <span className={styles.roomChip}>
                        Twin: <strong>${row.twinPrice}</strong>
                      </span>
                      <span className={styles.roomChip}>
                        Pvt: <strong>${row.privatePrice}</strong>
                      </span>
                    </div>
                  </td>
                  ...
                  <td>
                    <div className={styles.actionBtns}>
                      <Link
                        href={`/admin/yogacourse/500hourscourse/500hr-seats/${row._id}`}
