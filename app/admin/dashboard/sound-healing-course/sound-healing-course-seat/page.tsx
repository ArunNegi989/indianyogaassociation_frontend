// FILE: src/app/admin/dashboard/sound-healing/sound-healing-seats/page.tsx
"use client";
...
export default function SoundHealingSeatsListPage() {
  ...
  const fetchRows = async () => {
    try {
      const res = await api.get("/sound-healing-seats/get-all-batches");
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/sound-healing-seats/delete-batch/${deleteId}`);
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.pageHeader}>
        <div>
          <h1 className={styles.pageTitle}>Sound Healing — Seats & Dates</h1>
          <p className={styles.pageSubtitle}>Manage upcoming batch dates, fees and seat availability</p>
        </div>
        <Link href="/admin/dashboard/sound-healing-course/sound-healing-course-seat/add-new" className={styles.addBtn}>
                ...
                <tr key={row._id} className={styles.row}>
                  ...
                  <td>
                    <div className={styles.actionBtns}>
                      <Link href={`/admin/dashboard/sound-healing-course/sound-healing-course-seat/${row._id}`} className={styles.editBtn}>
                        ✎ Edit
                      </Link>
