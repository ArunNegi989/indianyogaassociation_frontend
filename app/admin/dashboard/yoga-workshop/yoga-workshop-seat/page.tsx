...
export default function MeditationSeatsListPage() {
  ...
  const fetchRows = async () => {
    try {
      const res = await api.get("/meditation-seats/get-all-batches");
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      await api.delete(`/meditation-seats/delete-batch/${deleteId}`);
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.pageHeader}>
        <div>
          <h1 className={styles.pageTitle}>Meditation Workshop — Seats & Dates</h1>
          <p className={styles.pageSubtitle}>Manage upcoming batch dates, fees and seat availability</p>
        </div>
        <Link href="/admin/dashboard/yoga-workshop/yoga-workshop-seat/add-new" className={styles.addBtn}>
                ...
                <tr key={row._id} className={styles.row}>
                  ...
                  <td>
                    <div className={styles.actionBtns}>
                      <Link href={`/admin/dashboard/yoga-workshop/yoga-workshop-seat/${row._id}`} className={styles.editBtn}>
                        ✎ Edit
                      </Link>
