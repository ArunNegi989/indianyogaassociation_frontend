...
══════════════════════════════════════ */
const IMGS = {
  yoga1: "https://images.unsplash.com/photo-1599901860904-17e6ed7083a0?w=900&q=80",
  ...
  yoga4: "https://images.unsplash.com/photo-1506126613408-eca07ce68773?w=900&q=80",
  yoga5: "https://images.unsplash.com/photo-1518611012118-696072aa579a?w=900&q=80",
  yoga6: "https://images.unsplash.com/photo-1593811167562-9cef47bfc4d7?w=900&q=80",
  college: "https://images.unsplash.com/photo-1503676260728-1c00da094a0b?w=900&q=80",
  rishikesh: "https://images.unsplash.com/photo-1501854140801-50d01698950b?w=900&q=80",
  career: "https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=900&q=80",
...
const tabs = [
  ...
  },
  {
    ...
    content: {
      ...
      syllabus: {
        theory: ["Philosophical Foundation of Yoga - 30 Marks", "Principles and Practices of Yoga in Traditional Texts - 30 Marks", "Allied Science - 30 Marks", "Applied Yoga - 30"],
        practical: ["Demonstration Skills - 15", "Teaching Skills - 15", "Evaluation Skills - 20", "Application of knowledge - 20", "Field Experience - 10"],
      },
    },
  },
];
  ...
  return (
    <svg viewBox="0 0 300 300" width={size} height={size} xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      ...
      <g fill="none" stroke={c2} strokeWidth={sw*0.65} opacity="0.45">
        {([[150,5,150,295],[5,150,295,150],[47,47,253,253],[253,47,47,253]] as number[][]).map((d,i) => (
...
══════════════════════════════════════ */
function TextImageRow({ children, imageUrl, imageAlt, badge, reverse = false }: {
  children: React.ReactNode; imageUrl: string; imageAlt: string; badge?: string; reverse?: boolean;
}) {
  return (
...
function TextVideoRow({ children, ytId, reverse = false }: {
  children: React.ReactNode; ytId: string; reverse?: boolean;
}) {
  return (
    <div className={`${styles.tiRow} ${reverse ? styles.tiRowReverse : ""}`}>
      <div className={styles.tiText}>{children}</div>
...
/* ══════════════════════════════════════
   SYLLABUS GRID
══════════════════════════════════════ */
function SyllabusGrid({ theory, practical }: { theory: string[]; practical: string[] }) {
  return (
      ...
      <div className={styles.syllabusCol}>
        ...
        {theory.map((t, i) => (
          <div key={i} className={styles.syllabusItem}>
            <span className={styles.syllabusCheck}>✓</span>{t}
          </div>
        ))}
      </div>
      <div className={styles.syllabusCol}>
        ...
        {practical.map((p, i) => (
          <div key={i} className={styles.syllabusItem}>
            <span className={styles.syllabusCheck}>✓</span>{p}
          </div>
        ))}
      </div>
    </div>
...
function AimImagesStrip({ images }: { images: string[] }) {
  return (
    <div className={styles.aimImagesStrip}>
      {images.map((src, i) => (
        <div key={i} className={styles.aimImageCard} style={{ animationDelay: `${i * 0.12}s` }}>
  ...
  if (c.isYogaMaster) {
    return (
      <div className={styles.tabPane}>
        ...
        <div className={styles.yogaMasterHero}>
          <div className={styles.yogaMasterLeft}>
            ...
            {c.details.map((d: any, i: number) => (
              <p key={i} className={styles.tabBody}><strong className={styles.tabStrong}>{d.label}:</strong> {d.text}</p>
            ))}
            <div className={styles.yogaMasterEligTitle}>Requirement / Eligibility</div>
            <ol className={styles.tabOl}>
              {c.eligibility.map((e: string, i: number) => <li key={i}>{e}</li>)}
            </ol>
          </div>
          <div className={styles.yogaMasterRight}>
            <div className={styles.yogaMasterVideoWrap}>
              ...
              <div className={styles.yogaMasterVideoBadge}><PulseDot /> Live Training</div>
            </div>
          </div>
        </div>
        ...
        <SyllabusGrid theory={c.syllabus.theory} practical={c.syllabus.practical} />
    ...
    <div className={styles.tabPane}>
      ...
      <div className={styles.tabIntroRow}>
        <div className={styles.tabIntroText}>
          ...
          <p className={styles.tabBody}><strong className={styles.tabStrong}>Affiliation:</strong> {c.affiliation}</p>
        </div>
        <div className={styles.tabIntroVideo}>
          <div className={styles.tabVideoFrame}>
            <AutoVideo ytId={YT_ID} className={styles.tabVideoIframe} />
            <div className={styles.tabVideoBadge}><PulseDot /> Daily Classes</div>
          </div>
        </div>
      </div>
      ...
      {c.sections.map((sec: any, i: number) => (
        <div key={i} className={styles.tabSection}>
          ...
          {sec.extra && <p className={styles.tabBodyExtra}><strong>{sec.extra}</strong></p>}
          {sec.items && (
          ...
          {sec.theory && <SyllabusGrid theory={sec.theory} practical={sec.practical} />}
        </div>
      ))}
...
══════════════════════════════════════ */
function CourseCard({ course, index }: { course: (typeof inPersonCourses)[0]; index: number }) {
  ...
  return (
    <div className={`${styles.courseCard} ${isReverse ? styles.courseCardReverse : ""}`}>
      <div className={styles.courseCardImgWrap}>
        <Image src={course.image} alt={course.title} fill className={styles.courseCardImg} />
        <div className={styles.courseCardImgOverlay} />
        <div className={styles.courseCardBadge} style={{ background: course.color }}>{course.badge}</div>
        <div className={styles.courseCardHoursTag}>{course.duration}</div>
      </div>
      <div className={styles.courseCardBody}>
        <h3 className={styles.courseCardTitle}>{course.title}</h3>
        <div className={styles.courseCardUnderline} style={{ background: course.color }} />
        ...
        <a href="#" className={styles.bookSpotBtn} style={{ background: `linear-gradient(135deg, ${course.color}, #a85800)` }}>
          Book Your Spot →
        </a>
      </div>
    </div>
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.heroSection}>
        <Image src={heroImg} alt="Yoga College in Rishikesh" width={1460} height={580} className={styles.heroImage} priority />
      </section>
      ...
      <section className={styles.heroTitleSection}>
        <div className={styles.container}>
          ...
          <p className={styles.heroSubtitle}>AYM Yoga School — Ministry of AYUSH Certified Programs</p>
        </div>
      </section>
      ...
      <section className={styles.tabsSection}>
        <div className={styles.container}>
          ...
          <div className={styles.tabContentWrap}>
            {tabs.map((tab) => activeTab === tab.id ? <TabContent key={tab.id} tab={tab} /> : null)}
          </div>
        </div>
      </section>
        ...
        <div className={styles.fullVideoWrap}>
          ...
          <div className={styles.fullVideoOverlay}>
            <div className={styles.fullVideoTextWrap}>
              <div className={styles.fullVideoBadge}><PulseDot /> Live Yoga Classes</div>
              <h2 className={styles.fullVideoTitle}>Experience the Energy of Rishikesh</h2>
              <p className={styles.fullVideoSub}>Watch our students transform their practice in the yoga capital of the world</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className={styles.coursesSection}>
        <div className={styles.container}>
          ...
          <p className={styles.coursesSub}>Ministry of AYUSH certified programs with accommodation, meals & certification included</p>
      ...
      <section className={styles.collegeSection}>
        <div className={styles.container}>
          ...
          <TextImageRow
            ...
            badge="Est. 2016"
          >
            <VintageHeading center={false}>About AYM Yoga College</VintageHeading>
            <p className={styles.bodyPara}>
              AYM Yoga College was established on 21 June 2016 to spread the quality of higher yoga education in India.
              We are honored to inform everyone that AYM Yoga institutions has started AYM Yoga College (Yoga Mahavidhyalaya)
              in Rishikesh, India. The specialty of this college will be quality of teaching, research-oriented educational classes,
              participation of students in research projects, latest and up to date yoga and meditational practices, debates on yoga
              and meditational topics, guest lectures by known personalities from the field of yoga and meditation, job placements
              on national and international level.
            </p>

            <div className={styles.collegeHighlights}>
              {["Quality Teaching", "Research-Oriented", "Guest Lectures", "Job Placement"].map((h, i) => (
            ...
            <div className={styles.collegeCourseGrid}>
              {collegeCourses.map((c, i) => (
                <div key={i} className={styles.collegeCourseCard}>
                  <span className={styles.collegeCourseNum}>{String(i + 1).padStart(2, "0")}</span>
          ...
          <TextImageRow
            ...
            reverse={true}
          >
            <VintageHeading center={false}>Master of Yoga (M.A. Yoga) — Objectives</VintageHeading>
      ...
      <section className={styles.maSection}>
        <div className={styles.container}>
          ...
          <TextVideoRow ytId={YT_ID} reverse={false}>
            <>
              <VintageHeading center={false}>Eligibility for M.A. Yoga / PG Diploma</VintageHeading>
              <p className={styles.bodyPara}>Bachelor's Degree from any University in any subject.</p>
              ...
              <p className={styles.bodyPara}>
                You can buy the prospectus for MA and PG Diploma from AYM Yoga College office and submit it on given date
                along with admission fee. Admission will be based as per Uttarakhand Sanskrit University norms.
              </p>
            </>
          </TextVideoRow>
        </div>
      ...
      <section className={styles.careerSection}>
        <div className={styles.container}>
          ...
          >
            <VintageHeading center={false}>Career Option after Diploma &amp; Masters</VintageHeading>
            <p className={styles.bodyPara}>
              The courses of Yoga and Meditation open a vast possibility and opportunities for job seeking aspirants.
              After completion of course, people can work in hospitals, health centers, health clubs or can practice on their own
              as yoga and meditation experts. They are also free to teach the same in colleges and universities.
            </p>
            <p className={styles.bodyPara}>
              They can also start their career as research associate/scholar, research analyst, consultant, freelancer or even
              certified instructor. There are vacancies in yoga, meditation and other related fields, which keep appearing online
              and offline and one needs to keep oneself updated to get the job of interest.
            </p>
            <p className={styles.bodyPara}>
              One can also start own work by becoming health advisor/counsellor and can offer people, their expert advice and services.
              Thus, this course has great potential when it comes to having good career prospects.
            </p>
            <div className={styles.careerOptions}>
              {["Hospital & Health Centers","Yoga Studios","Colleges & Universities","Research Scholar","Freelancer / Consultant","Own Yoga Center"].map((o, i) => (
                <div key={i} className={styles.careerChip}>
                  <span className={styles.careerChipDot} />{o}
                </div>
              ))}
            </div>
          </TextImageRow>
