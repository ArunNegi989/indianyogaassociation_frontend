...
interface StatItem { icon: string; value: string; title: string; desc: string; }
interface ModuleItem { title: string; intro: string; items: string[]; body?: string; }

interface Content1 {
  _id: string; slug: string; status: string;
  pageMainH1: string; heroImage: string; heroImgAlt: string;
  introPara1: string; introPara2: string; introPara3: string; introPara4: string;
  stats: StatItem[];
  aimsH3: string; aimsIntro: string; aimsOutro: string;
  aimsKeyObjLabel: string; aimsBullets: string[];
  overview: { h2: string; certName: string; level: string; eligibility: string; minAge: string; credits: string; language: string; };
  upcomingDatesH2: string; upcomingDatesSubtext: string;
  feeIncludedTitle: string; feeNotIncludedTitle: string;
  includedFee: string[]; notIncludedFee: string[];
  syllabusH3: string; syllabusIntro: string; modules: ModuleItem[];
  ashtanga: { h2: string; subtitle: string; desc: string; image: string; imgAlt: string; pills: string[]; };
  primary: { h3: string; subtext: string; intro: string; foundationItems: string[]; weekGrid: Array<{ week: string; icon: string; items: Array<{ t: string; d: string }>; [key: string]: any; }>; };
  hatha: { h2: string; subtitle: string; desc: string; image: string; imgAlt: string; pills: string[]; asanas: Array<{ n: number; name: string; sub: string; filter?: string; }>; };
}

interface Content2 {
  evalH2: string; evalDesc: string;
  luxuryH2: string; luxFeatures: string[]; luxImages: string[];
  indianFeeH2: string; indianFees: Array<{ label: string; price: string }>;
  scheduleH2: string; schedDesc: string;
  schedRows: Array<{ time: string; activity: string }>; schedImages: string[];
  moreInfoH2: string; instrLangs: Array<{ lang: string } | string>;
  visaPassportDesc: string;
  programs: Array<{ title: string; desc: string; duration: string; start: string; oldPrice: string; price: string; }>;
  requirementsH2: string; reqImage: string;
  faqItems: Array<{ q: string; a: string }>;
  knowQA?: Array<{ q: string; a: string }>;
  bookingH2: string;
  step1Icon?: string; step1Title?: string; bookingStep1Desc?: string;
  step2Icon?: string; step2Title?: string; bookingStep2Desc?: string;
  step3Icon?: string; step3Title?: string; bookingStep3Desc?: string;
  step4Icon?: string; step4Title?: string; bookingStep4Desc?: string;
}

interface Batch {
  _id: string; startDate: string; endDate: string;
  usdFee: string; inrFee: string;
  dormPrice: number; twinPrice: number; privatePrice: number;
  totalSeats: number; bookedSeats: number; note?: string;
}
...
const StyleIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    <circle cx="12" cy="4" r="1.5" /><path d="M12 6v5.5" /><path d="M8.5 13c0 2 1.5 4 3.5 4.5 2-0.5 3.5-2.5 3.5-4.5" /><path d="M10 18l-1.5 3.5M14 18l1.5 3.5" /><path d="M7 11l5 2.5 5-2.5" />
  </svg>
);
const LangIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    <circle cx="12" cy="12" r="9" /><path d="M2 12h20" /><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" />
  </svg>
);
const DateIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    <rect x="3" y="4" width="18" height="18" rx="2" /><path d="M16 2v4M8 2v4M3 10h18" /><circle cx="8" cy="15" r="1" fill="currentColor" /><circle cx="12" cy="15" r="1" fill="currentColor" /><circle cx="16" cy="15" r="1" fill="currentColor" />
  </svg>
);
    ...
    <div className={styles.videoSection}>
      <div className={styles.videoCard}>
        ...
        <div className={styles.videoBadge}>
          ...
          <span className={styles.videoBadgeTxt}>Yoga Teacher Training · Rishikesh</span>
        </div>
      </div>
    </div>
  );
...
function CourseInfoCard({ batches }: { batches: Batch[] }) {
  ...
  const startingPrice = available.length > 0 ? Math.min(...available.map((b) => b.dormPrice)) : 699;
    ...
    <div className={styles.icWrap}>
      <div className={styles.icCard}>
        <div className={styles.icLeft}>
          <div className={styles.icHdr}><span className={styles.icHdrTxt}>COURSE DETAILS</span></div>
          ...
          <>
            <ul className={styles.moduleOl}>
              {(expanded ? items : items.slice(0, 4)).map((it, i) => (
                <li key={i}><span className={styles.moduleOlDot} />{stripHtml(it)}</li>
              ))}
            </ul>
            {items.length > 4 && (
...
function IncludeExcludeTabs({ includedFee, notIncludedFee, includedTitle, notIncludedTitle }: {
  includedFee: string[]; notIncludedFee: string[]; includedTitle: string; notIncludedTitle: string;
}) {
  const [activeTab, setActiveTab] = useState<"include" | "exclude">("include");
  return (
    <div className={styles.incWrap}>
      <div className={styles.incTabs}>
        <button className={`${styles.incTab} ${activeTab === "include" ? styles.active : ""}`} onClick={() => setActiveTab("include")}>✓ {includedTitle || "What Is Included?"}</button>
        <button className={`${styles.incTab} ${activeTab === "exclude" ? styles.active : ""}`} onClick={() => setActiveTab("exclude")}>✕ {notIncludedTitle || "What Is Not Included?"}</button>
      </div>
...
function PremiumSeatBooking({ seats, currency, onCurrencyChange, rate, rateLoading }: {
  seats: Batch[]; currency: Currency; onCurrencyChange: (c: Currency) => void; rate: number; rateLoading: boolean;
}) {
  ...
  function CurrencyDropdown({ currency, onChange }: { currency: Currency; onChange: (c: Currency) => void }) {
    ...
    return (
      <div className={styles.currDrop} ref={ref}>
        <button className={styles.currDropBtn} onClick={() => setOpen((p) => !p)} aria-expanded={open} type="button">
        ...
        {open && (
          <div className={styles.currDropMenu} role="listbox">
            {(["USD", "INR"] as Currency[]).map((c) => (
              <button key={c} className={`${styles.currDropItem} ${currency === c ? styles.currDropItemActive : ""}`} onClick={() => { onChange(c); setOpen(false); }} role="option" aria-selected={currency === c} type="button">
  ...
  return (
    <section className={styles.datesSection} id="dates-fees">
      ...
      <div className={styles.psbOrnLine}>
        <div className={styles.psbOrnL} /><div className={styles.psbOrnDiamond} /><div className={styles.psbOrnR} />
      </div>
      <div className={styles.psbLayout}>
        {/* LEFT PANEL */}
        <div className={styles.psbLeftPanel}>
          <div className={`${styles.psbCn} ${styles.psbCnTl}`} /><div className={`${styles.psbCn} ${styles.psbCnTr}`} />
          <div className={`${styles.psbCn} ${styles.psbCnBl}`} /><div className={`${styles.psbCn} ${styles.psbCnBr}`} />
          ...
          {rateLoading && (
            <div className={styles.rateLoader}><div className={styles.rateLoaderDot} /><span>Loading live exchange rate...</span></div>
          )}
          {seats.length === 0 ? (
            ...
            <div className={styles.psbBatchGrid}>
              {seats.map((batch) => {
                ...
                const full = rem <= 0; const low = !full && rem <= 5;
                ...
                return (
                  <div key={batch._id} className={[styles.psbBc, full ? styles.psbBcFull : "", isSelected ? styles.psbBcSel : ""].filter(Boolean).join(" ")} onClick={() => { if (!full) setSelectedId(batch._id); }}>
                    <div className={styles.psbBcTick}><svg viewBox="0 0 10 10" fill="none"><polyline points="1.5,5 4,7.5 8.5,2.5" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg></div>
                    ...
                    <div className={styles.psbBcStatus}><div className={`${styles.psbBcDot} ${dotCls}`} /><span className={`${styles.psbBcStxt} ${txtCls}`}>{statusTxt}</span></div>
                    {!full && (
                      <>
                        <div className={styles.psbBcSeatsBar}><div className={styles.psbBcSeatsBarFill} style={{ width: `${seatsPercent}%`, background: low ? "linear-gradient(90deg,#c8700a,#e09030)" : "linear-gradient(90deg,#3d6000,#6aa000)" }} /></div>
        ...
        <div className={styles.psbRightPanel}>
          ...
          <div className={styles.psbRpHead}>
            ...
            <div className={styles.psbRpDur}>
              <svg width="11" height="11" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="7" stroke="rgba(255,243,210,0.8)" strokeWidth="1.2" /><path d="M8 4.5V8.5L10.5 10" stroke="rgba(255,243,210,0.8)" strokeWidth="1.2" strokeLinecap="round" /></svg>
          ...
          <div className={styles.psbRpBody}>
            ...
            {selected && currency === "USD" && (
              <div className={styles.psbInrRow}><span className={styles.psbInrLbl}>Indian Price</span><span className={styles.psbInrAmt}>{selected.inrFee}</span></div>
            )}
            {selected && currency === "INR" && (
              <div className={styles.psbInrRow}><span className={styles.psbInrLbl}>USD Price</span><span className={styles.psbInrAmt}>${selected.dormPrice} USD</span></div>
            )}
            ...
            {selected && (() => {
              ...
              return (
                <div className={styles.psbRpSeatsWrap}>
                  ...
                  <div className={styles.psbRpSeatsBar}><div className={styles.psbRpSeatsBarFill} style={{ width: `${pct}%`, background: full ? "#8a2c00" : low ? "linear-gradient(90deg,#c8700a,#e09030)" : "linear-gradient(90deg,#3d6000,#6aa000)" }} /></div>
                </div>
              );
            })()}
            <div className={styles.psbSelDisplay}>
              {selected ? (
                <><div className={styles.psbSelLabel}>Selected Batch</div><div className={styles.psbSelDate}>{shortDateRange(selected.startDate, selected.endDate)}, {monthYear(selected.startDate)}</div></>
              ) : (
            ...
            {selected ? (
              <a href={`/yoga-registration?batchId=${selected._id}&type=200hr`} className={styles.psbBookBtn}>
                ...
                <svg className={styles.psbArrowIcon} viewBox="0 0 16 16" fill="none"><path d="M3 8h10M9 4l4 4-4 4" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
              </a>
            ) : (
...
function useCurrencyRate() {
  ...
  useEffect(() => {
    fetch("https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@latest/v1/currencies/usd.json")
      .then((r) => r.json())
      .then((data) => { const inr = data?.usd?.inr; if (inr) setRate(inr); })
...
function PageSkeleton() {
  return (
    <div className={styles.root} style={{ padding: "4rem 2rem", textAlign: "center" }}>
      ...
      <p style={{ color: "#e07b00", fontSize: "1.2rem" }}>Loading yoga journey...</p>
    </div>
  );
}
...
export default function TwoHundredHourYoga() {
  ...
  if (error) return (<div className={styles.root} style={{ padding: "4rem", textAlign: "center" }}><p style={{ color: "red" }}>{error}</p></div>);

  const allAsanas = content1?.hatha?.asanas || [];
  const modules = content1?.modules || [];
  const filteredAsanas = asanaFilter === "All Poses" ? allAsanas : allAsanas.filter((a) => (a.filter || "All Poses") === asanaFilter);

  const bookingSteps = [
    { icon: content2?.step1Icon, title: content2?.step1Title, text: content2?.bookingStep1Desc },
    { icon: content2?.step2Icon, title: content2?.step2Title, text: content2?.bookingStep2Desc },
    { icon: content2?.step3Icon, title: content2?.step3Title, text: content2?.bookingStep3Desc },
    { icon: content2?.step4Icon, title: content2?.step4Title, text: content2?.bookingStep4Desc },
  ].filter((step) => step.title || step.text);

  // Overview items array for rendering
  const overviewItems = content1?.overview ? [
    { key: "certName", label: "Name of the certification", value: content1.overview.certName },
    { key: "level", label: "Course level", value: content1.overview.level },
    { key: "eligibility", label: "Requirement/Eligibility", value: content1.overview.eligibility },
    { key: "minAge", label: "Minimum age", value: content1.overview.minAge },
    { key: "credits", label: "Credit points for certificate", value: content1.overview.credits },
    { key: "language", label: "Language", value: content1.overview.language },
  ...
  return (
    <div className={styles.root}>
      ...
      <section id="hero" className={styles.heroSection}>
        {content1?.heroImage && (
          <Image src={imgUrl(content1.heroImage)} alt={content1.heroImgAlt || "200 Hour Yoga Teacher Training"} width={1180} height={540} className={styles.heroImage} priority />
        )}
      </section>
      ...
      <section className={styles.heroSection2}>
        ...
        <VintageHeading
          para={content1?.introPara1 ? <span className={styles.bodyText}>{stripHtml(content1.introPara1)}</span> : undefined}
        >
        ...
        {[content1?.introPara2, content1?.introPara3, content1?.introPara4].filter(Boolean).map((para, i) => (
      ...
      <section id="dates-fees" className={styles.contentSection}>
        ...
        {content1?.aimsH3 && (
          <div className={styles.h3LeftWrap}>
            <div className={styles.h3LeftContent}>
              ...
              {content1?.aimsKeyObjLabel && <p className={styles.bodyText}><strong>{stripHtml(content1.aimsKeyObjLabel)}</strong></p>}
        ...
        {content1?.overview && (
          <>
            <VintageHeading para={<span className={styles.bodyText}>A comprehensive certification program designed to transform passionate practitioners into confident, knowledgeable yoga teachers.</span>}>
  ...
  <div className={styles.overviewItemText}>
    ...
    {stripHtml(item.value)}
      ...
      {/* ════ PREMIUM SEAT BOOKING ════ */}
      <PremiumSeatBooking seats={batches} currency={currency} onCurrencyChange={setCurrency} rate={rate} rateLoading={rateLoading} />
      ...
      <section id="curriculum" className={styles.contentSection2}>
        ...
        <div className={styles.moduleGrid}>
          {modules.slice(0, 4).map((mod, i) => <ModuleCard key={i} title={mod.title} intro={mod.intro} items={mod.items} index={i} />)}
        </div>
      </section>
      ...
      <section id="inclusions" className={styles.contentSection}>
        <div className={styles.moduleGrid}>
          {modules.slice(4, 8).map((mod, i) => <ModuleCard key={i} title={mod.title} intro={mod.intro} items={mod.items} index={i + 4} />)}
        </div>

        {content1?.ashtanga && (
          <>
            <VintageHeading para={content1.ashtanga.subtitle ? <span className={styles.bodyText}>{stripHtml(content1.ashtanga.subtitle)}</span> : undefined}>
              {stripHtml(content1.ashtanga.h2) || "Module 8.1: Ashtanga Vinyasa Yoga"}
            </VintageHeading>
            <div className={styles.moduleDetailGrid}>
              <div className={styles.moduleDetailImg}>
                {content1.ashtanga.image && <img src={imgUrl(content1.ashtanga.image)} alt={content1.ashtanga.imgAlt || "Ashtanga"} className={styles.modImg} />}
              </div>
              <div>
                ...
                {content1.ashtanga.pills?.filter(Boolean).length ? (
                  <div className={styles.featurePills}>
                    {content1.ashtanga.pills.filter(Boolean).map((pill, i) => <span key={i} className={styles.pill}>{stripHtml(pill)}</span>)}
                  </div>
                ) : null}
              </div>
            </div>
      ...
      <section className={styles.contentSection3}>
              ...
              <div className={styles.primaryCurrCard}>
                ...
                {content1.primary.foundationItems?.length ? (
                  <div className={styles.foundationBox}>
                    <div className={styles.foundationHeader}><span className={styles.foundIcon}>📖</span><strong>Foundation</strong></div>
                    <ul className={styles.foundList}>{content1.primary.foundationItems.map((it, i) => <li key={i}>{stripHtml(it)}</li>)}</ul>
                  </div>
                ) : null}
                {content1.primary.weekGrid?.length ? (
                  <div className={styles.weekGrid}>
                    {content1.primary.weekGrid.map((w, i) => {
                      const items = Object.keys(w).filter((key) => key.startsWith("t")).sort((a, b) => Number(a.replace("t", "")) - Number(b.replace("t", ""))).map((key) => { const index = key.replace("t", ""); return { t: w[key], d: w[`d${index}`] }; }).filter((item) => item.t && item.d);
                      return (
                        <div key={i} className={styles.weekCard}>
                          ...
                          {items.map((it, j) => (
                            <div key={j} className={styles.weekItem}>
                              ...
                              <div><strong>{stripHtml(it.t)}</strong><br /><span className={styles.weekDesc}>{stripHtml(it.d)}</span></div>
                            </div>
                          ))}
                        </div>
                      );
        ...
        {content1?.hatha && (
          <>
            <VintageHeading para={content1.hatha.subtitle ? <span className={styles.bodyText}>{stripHtml(content1.hatha.subtitle)}</span> : undefined}>
            ...
            <div className={styles.moduleDetailGrid}>
              ...
              <div>
                ...
                {content1.hatha.pills?.filter(Boolean).length ? (
                  <div className={styles.featurePills}>
                    {content1.hatha.pills.filter(Boolean).map((pill, i) => <span key={i} className={styles.pill}>{stripHtml(pill)}</span>)}
                  </div>
                ) : null}
              </div>
            </div>
      ...
      {allAsanas.length > 0 && (
        <section className={styles.contentSection}>
          <VintageHeading para={<span className={styles.centerSubtext}>Master these {allAsanas.length} essential postures as part of your comprehensive training</span>}>
            Hatha Yoga Asanas
          </VintageHeading>
          <div className={styles.asanaFilterRow}>
            {ASANA_FILTERS.map((f) => (
              <button key={f} className={`${styles.filterBtn} ${asanaFilter === f ? styles.filterActive : ""}`} onClick={() => setAsanaFilter(f)}>
                {f}
                {f !== "All Poses" && <span style={{ marginLeft: 4, opacity: 0.7, fontSize: "0.75em" }}>({allAsanas.filter((a) => (a.filter || "All Poses") === f).length})</span>}
              </button>
            ))}
          </div>
      ...
      <section className={styles.contentSection}>
        {content2?.luxuryH2 && (
          <>
            <VintageHeading para={<span className={styles.bodyText}>Experience world-class facilities designed to support your yoga journey with comfort and serenity.</span>}>
        ...
        {content2?.scheduleH2 && (
          <>
            <VintageHeading para={content2.schedDesc ? <span className={styles.bodyText}>{stripHtml(content2.schedDesc)}</span> : undefined}>
            ...
            <div className={styles.schedLayout}>
              <div className={styles.schedTableWrap}>
                ...
                <table className={styles.schedTable}>
                  <thead><tr><th>Time</th><th>Schedule</th></tr></thead>
                  <tbody>
                    {(content2.schedRows || []).map((r, i) => (
                      <tr key={i}><td>{stripHtml(r.time)}</td><td>{stripHtml(r.activity)}</td></tr>
                    ))}
                  </tbody>
                </table>
              </div>
              ...
              <div className={styles.schedImgGrid}>
                {content2.schedImages?.length ? (
                  <img src={imgUrl(content2.schedImages[0])} alt="Yoga class schedule" className={styles.schedImg} />
                ) : (
                  <img src="https://images.unsplash.com/photo-1545205597-3d9d02c29597?w=600&h=800&fit=crop" alt="Yoga class" className={styles.schedImg} />
                )}
              </div>
            </div>
          </>
      ...
      {/* ════ MORE INFO + CTA ════ */}
      <section className={styles.contentSection}> <VintageHeading>Course Fee Inclusions</VintageHeading>
        <IncludeExcludeTabs includedFee={content1?.includedFee || []} notIncludedFee={content1?.notIncludedFee || []} includedTitle={content1?.feeIncludedTitle || ""} notIncludedTitle={content1?.feeNotIncludedTitle || ""} />
        {content2?.moreInfoH2 && (
      ...
      {content2?.programs?.length ? (
        <section className={styles.contentSection}>
          <VintageHeading para={<span className={styles.centerSubtext}>Expand your teaching expertise with our specialized certification combinations</span>}>
      ...
      {content2?.requirementsH2 && (
       <section className={styles.contentSection}>
  <VintageHeading>
    {stripHtml(content2.requirementsH2)}
  </VintageHeading>
    ...
    {content2.knowQA?.slice(2).map((item, i) => (
      ...
      <section className={styles.contentSection}>
        {(content2?.bookingH2 || bookingSteps.length) ? (
          <>
            <VintageHeading>{stripHtml(content2?.bookingH2 || "How to book your spot?")}</VintageHeading>
                ...
                {bookingSteps.map((s, i) => (
                  <div key={i} className={styles.bookingStep}>
                    <div className={styles.bookingStepIconWrap}>
                      <span className={styles.bookingStepIcon}>{s.icon || ["🔍","📝","💳","✅"][i] || "🧘"}</span>
                    </div>
                    <div className={styles.bookingStepCard}>
                      <div className={styles.bookingStepNum}>Step {i + 1}</div>
                      <div className={styles.bookingStepTitle}>{stripHtml(s.title || "")}</div>
                      <p className={styles.bookingStepText}>{stripHtml(s.text || "")}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {content2.faqItems.map((faq, i) => (
                <div key={i} className={styles.faqItem}>
                  <button className={styles.faqBtn} onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                    ...
                    <span className={styles.faqIcon} style={{ transform: openFaq === i ? "rotate(45deg)" : "rotate(0)" }}>
                      {openFaq === i ? "×" : "+"}
                    </span>
                  </button>
                  {openFaq === i && (
